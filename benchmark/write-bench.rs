...
use turbolite::tiered::{
    CacheConfig, CompressionConfig, PrefetchConfig, TurboliteConfig, TurboliteSharedState,
    TurboliteVfs,
};
...
impl WriteBenchCtx {
    ...
    fn reset_s3_counters(&self) {
        ...
        self.put_count_base
            .store(self.s3.put_count(), Ordering::Relaxed);
        self.bytes_put_base
            .store(self.s3.bytes_put(), Ordering::Relaxed);
    }
    ...
    fn s3_put_counters(&self) -> (u64, u64) {
        let pc = self
            .s3
            .put_count()
            .saturating_sub(self.put_count_base.load(Ordering::Relaxed));
        let pb = self
            .s3
            .bytes_put()
            .saturating_sub(self.bytes_put_base.load(Ordering::Relaxed));
        (pc, pb)
    }
...
fn generate_payload(id: i64) -> Vec<u8> {
    ...
    (0..len)
        .map(|i| phash(id as u64 + i as u64) as u8)
        .collect()
}
...
fn scenario_sustained(cli: &Cli) {
    ...
    println!(
        ...
        format_number(cli.rows),
        format_number(cli.batch_size),
        cli.autocheckpoint,
        cli.page_size,
        cli.ppg
    );
    ...
    println!("\n--- Results ---");
    println!(
        "  Insert time:         {:.1}ms ({:.0} rows/sec including auto-checkpoints)",
        overall_ms,
        cli.rows as f64 / (overall_ms / 1000.0)
    );
    println!(
        "  Auto-checkpoint S3:  {} PUTs, {}",
        auto_puts,
        format_bytes(auto_bytes)
    );
    println!(
        "  Final checkpoint:    {:.1}ms, {} PUTs, {}",
        cp.wall_ms,
        cp.s3_puts,
        format_bytes(cp.s3_bytes)
    );
    println!(
        "  DB: {} pages x {} = {:.1}MB ({} groups)",
        pages, psz, mb, groups
    );
...
fn scenario_checkpoint_latency(cli: &Cli) {
    ...
    let dirty_targets: Vec<usize> = cli
        .dirty_pages
        .split(',')
...
fn scenario_incremental(cli: &Cli) {
    ...
    println!(
        ...
        cli.cycles,
        format_number(cli.rows_per_cycle),
        cli.page_size,
        cli.ppg
    );
...
fn scenario_update(cli: &Cli) {
    ...
    println!(
        "    insert: {:.1}ms, cp: {:.1}ms ({} PUTs, {}), {} pages ({} groups)",
        insert_ms,
        cp1.wall_ms,
        cp1.s3_puts,
        format_bytes(cp1.s3_bytes),
        p1,
        groups_for(p1, cli.ppg)
    );
    verify_cold_reader(&s.prefix, &s.db_name, cli, cli.rows);
    ...
    println!(
        "\n  Phase 2: UPDATE {} rows (10%, scattered)",
        format_number(update_count)
    );
    let t = Instant::now();
    {
    ...
    println!(
        "    update: {:.1}ms, cp: {:.1}ms ({} PUTs, {})",
        update_ms,
        cp2.wall_ms,
        cp2.s3_puts,
        format_bytes(cp2.s3_bytes)
    );
    ...
    // Verify cold reader sees updates
    {
        ...
        let updated: i64 = reader
            .query_row("SELECT COUNT(*) FROM items WHERE counter > 0", [], |r| {
                r.get(0)
            })
            .unwrap();
        ...
        assert!(
            ...
            "integrity_check failed after UPDATE"
        );
        println!(
            "    cold reader: {} total, {} updated, integrity OK",
            count, updated
        );
    }
    drop(s.conn);
...
fn scenario_delete(cli: &Cli) {
    ...
    println!(
        "\n  Phase 2: DELETE {} rows (50%)",
        format_number(delete_count)
    );
    let t = Instant::now();
    {
        let tx = s.conn.unchecked_transaction().unwrap();
        tx.execute("DELETE FROM items WHERE id % 2 = 0", [])
            .unwrap();
        tx.commit().unwrap();
    }
...
fn scenario_realistic(cli: &Cli) {
    ...
    println!(
        "  Seed: {} rows in {:.1}ms, cp {:.1}ms ({} PUTs), {} pages ({} groups)",
        format_number(cli.rows),
        seed_ms,
        cp_seed.wall_ms,
        cp_seed.s3_puts,
        p_seed,
        groups_for(p_seed, cli.ppg)
    );
    verify_cold_reader(&s.prefix, &s.db_name, cli, cli.rows);
        ...
        for op_i in 0..burst {
            ...
            match op_type {
                ...
                30..=54 => {
                    ...
                    tx.execute(
                        "UPDATE items SET value = ?1, counter = counter + 1 WHERE id = ?2",
                        rusqlite::params![format!("upd_r{}_o{}", round, op_i), target as i64],
                    )
                    .unwrap();
                    n_update += 1;
                }
                ...
                _ => {
                    ...
                    let mut stmt = tx
                        ...
                        .unwrap();
                    let _: i64 = stmt
                        .query_row(rusqlite::params![lo as i64, hi as i64], |r| r.get(0))
                        .unwrap_or(0);
                    n_select += 1;
                }
            }
    ...
    println!(
        "\n  Final: {} pages ({} groups), {:.1}MB",
        pages_final,
        groups_for(pages_final, cli.ppg),
        mb_final
    );
    drop(s.conn);
}
...
fn scenario_cold_write(cli: &Cli) {
    ...
    println!(
        "  Seeded {} rows ({} pages, {} groups) to S3",
        seed_rows,
        pages,
        groups_for(pages, cli.ppg)
    );
    drop(conn);
    ...
    cold_conn
        .execute(
            ...
            rusqlite::params![
                seed_rows as i64,
                generate_value(seed_rows as i64),
                0i64,
                generate_payload(seed_rows as i64)
            ],
        )
        .expect("cold write");
    ...
    cold_conn
        .execute(
            ...
            rusqlite::params![
                (seed_rows + 1) as i64,
                generate_value((seed_rows + 1) as i64),
                0i64,
                generate_payload((seed_rows + 1) as i64)
            ],
        )
        .expect("warm write");
    ...
    println!(
        "\n  First write (cold):  {:.1}ms ({} S3 GETs, {})",
        first_write_ms,
        first_gets,
        format_bytes(first_bytes)
    );
    println!(
        "  Second write (warm): {:.1}ms ({} S3 GETs, {})",
        second_write_ms,
        second_gets,
        format_bytes(second_bytes)
    );
    println!(
        "  Checkpoint:          {:.1}ms ({} PUTs, {})",
        cp.wall_ms,
        cp.s3_puts,
        format_bytes(cp.s3_bytes)
    );
...
fn scenario_merge_write(cli: &Cli) {
    let prefix = cli
        .prefix
        .as_ref()
        .expect("--prefix required for merge-write scenario");
    ...
    let existing_rows: i64 = conn
        ...
        .unwrap_or(0);
    println!(
        "  Existing DB: {} pages x {} = {:.1}MB, {} posts",
        pages_before,
        psz,
        mb_before,
        format_number(existing_rows as usize)
    );
    ...
    println!(
        "    S3 GETs (merge): {} requests, {}",
        cp_gets,
        format_bytes(cp_get_bytes)
    );
    println!(
        "    S3 PUTs (upload): {} requests, {}",
        cp.s3_puts,
        format_bytes(cp.s3_bytes)
    );
    ...
    println!(
        "  Batch write: {:.1}ms ({} S3 GETs, {})",
        batch_ms,
        batch_gets,
        format_bytes(batch_get_bytes)
    );
    ...
    println!(
        "    S3 GETs (merge): {} requests, {}",
        cp3_gets,
        format_bytes(cp3_get_bytes)
    );
    println!(
        "    S3 PUTs (upload): {} requests, {}",
        cp3.s3_puts,
        format_bytes(cp3.s3_bytes)
    );
    ...
    let total_rows: i64 = conn
        ...
        .unwrap_or(0);
    println!(
        "\n  Final: {} pages ({:.1}MB), {} posts",
        pages_after,
        mb_after,
        format_number(total_rows as usize)
    );
...
fn scenario_arctic_write(cli: &Cli) {
    let prefix = cli
        .prefix
        .as_ref()
        .expect("--prefix required for arctic-write scenario");
    ...
    println!(
        "    S3 GETs: {} requests, {}",
        gets,
        format_bytes(get_bytes)
    );
    println!(
        "    S3 PUTs: {} requests, {}",
        puts,
        format_bytes(put_bytes)
    );
...
fn scenario_two_phase(cli: &Cli) {
    let prefix = cli
        .prefix
        .as_ref()
        .expect("--prefix required for two-phase scenario");
    ...
    println!(
        "  Lock held: {:.1}ms (local WAL compaction only)",
        cp2.lock_ms
    );
    println!(
        "  S3 flush:  {:.1}ms (no lock, reads/writes can continue)",
        cp2.flush_ms
    );
    ...
    println!("\n--- Summary ---");
    println!(
        "  Blocking:   lock={:.1}ms (readers and writers blocked entire time)",
        cp1.wall_ms
    );
    println!(
        "  Two-phase:  lock={:.1}ms, flush={:.1}ms (readers/writers only blocked {:.1}ms)",
        cp2.lock_ms, cp2.flush_ms, cp2.lock_ms
    );
    println!(
        "  Lock reduction: {:.0}x faster lock release",
        if cp2.lock_ms > 0.0 {
            cp1.wall_ms / cp2.lock_ms
        } else {
            f64::INFINITY
        }
    );
