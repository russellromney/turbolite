...
fn open_db(path: &str, vfs: &str) -> *mut turbolite_ffi::ffi::TurboliteDb {
...
fn exec_sql(db: *mut turbolite_ffi::ffi::TurboliteDb, sql: &str) -> i32 {
...
fn query_json(db: *mut turbolite_ffi::ffi::TurboliteDb, sql: &str) -> String {
    let sql_c = CString::new(sql).unwrap();
